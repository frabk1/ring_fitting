...
def fit_limacon(points, xs, ys):
    """
    Fit limacon parameters (L1, L2, phi) with fixed center.

    Model:
      r(θ) = L1 * (1 + L2 * cos(θ - phi))
    ...
    * phi_est (float): estimated phi in radians.
    ...
    if not np.isfinite(L1_est) or L1_est <= 0:
        L1_est = float(np.median(r))
    L2_est = float(amp / L1_est) if L1_est != 0 else 0.0
    phi_est = float(np.arctan2(C, B))

    return L1_est, L2_est, phi_est
...
def _limacon_cost(points, xs, ys, L1, L2, phi):
    """
...
def general_fit(points, xs, ys, shape="circle", method="Powell",
    ...
    * res (dict): standardized fit result with keys:
    ...
    if shape == "circle":
        ...
        L1_est, L2_est, phi_est = fit_limacon(pts, xs, ys)
        params = (float(L1_est), float(L2_est), float(phi_est))
        cost = _limacon_cost(pts, xs, ys, L1_est, L2_est, phi_est)
